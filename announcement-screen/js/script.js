...
document.addEventListener("DOMContentLoaded", () => {
    ...
    console.log("selectedProperty do localStorage:", selectedProperty); // Verificando os dados no console
    ...
    if (selectedProperty) {
        ...
        if (selectedProperty.map_url) {
            ...
            // Caso o mapa não tenha URL no objeto, tente gerar um link a partir do endereço
            const endereco = `${selectedProperty.adress || ""}, ${selectedProperty.city || ""}`.trim();
            if (endereco) {
                const enderecoUrl = encodeURIComponent(endereco);
                mapIframe.src = `https://www.openstreetmap.org/export/embed.html?bbox=${enderecoUrl}`;
            } else {
                console.warn("Endereço não disponível para gerar o mapa.");
            }
        }
